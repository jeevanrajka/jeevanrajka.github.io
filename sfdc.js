...
var SFIDWidget = function() {
    ...
    function addLogin(targetDiv) {
        ...
        if(SFIDWidget.config.forgotPasswordEnabled === "true") {
            ...
            fp.href = decodeURIComponent(SFIDWidget.authconfig.LoginPage.ForgotPasswordUrl);
        ...
        if(SFIDWidget.authconfig.LoginPage.SelfRegistrationEnabled && (SFIDWidget.config.selfRegistrationEnabled === "true")) {
            ...
            sr.href = decodeURIComponent(SFIDWidget.authconfig.LoginPage.SelfRegistrationUrl);
        ...
        if (SFIDWidget.authconfig.AuthProviders.length > 0) {
            ...
            for (var i in SFIDWidget.authconfig.AuthProviders) {
                ...
                var hrefUrl = SFIDWidget.authconfig.AuthProviders[i].SsoUrl + '&startURL=' + encodeURIComponent(SFIDWidget.config.authorizeURL);
    ...
    return {
        init: function() {
            ...
            var brandTag = document.querySelector('meta[name="salesforce-brand"]');
            if (brandTag !== null) {
            ...
            if (communityURLTag === null) {
                ...
                if(SFIDWidget.config.brand) {
                    SFIDWidget.XAuthServerUrl += "&brand=" + encodeURIComponent(SFIDWidget.config.brand);
                }
            ...
            if (modeTag === null) {
                alert('Enter the mode for the salesforce-mode metatag, either inline, modal, or popup.');
                return;
            } else {
            ...
            if (redirect_uriTag === null) {
                alert('Enter the Callback URL for your client-side callback page, for example, https://:logindemo.herokuapp.com/_callback.php.');
                return;
            } else {
            ...
            if (loginHandlerTag === null) {
                alert('Enter the name of the JavaScript function to call on a successful login event for the salesforce-login-handler metatag.');
                return;
            } else {
